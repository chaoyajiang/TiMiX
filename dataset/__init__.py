...
from dataset.caption_dataset import re_train_dataset, re_eval_dataset, pretrain_dataset_4m, coco_dataset, nocaps_dataset, region_dataset
...
def create_dataset(dataset, config, epoch=None):
    ...
    if dataset=='pretrain':
        ...
        r_dataset = region_dataset(config = config, data_path = config['train_file_regions'],
                                 transform=pretrain_transform, box_transform=box_transform, load_local=True)
        print('return 2')
        return dataset, r_dataset
