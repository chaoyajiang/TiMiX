...
class TPMix(Mixup):
    ...
    """
    def __init__(self, mixup_alpha=1., prob=1.0, switch_prob=1.0,
                 mode='batch',  label_smoothing=0.1, num_classes=1000, min_side_ratio=0.25, max_side_ratio=0.75, side=14,patch_size=16):
    ...
    def tpmix(self, inputs,attn,rectangle_size):
        inputs_side = inputs.shape[2]
     
        inputs = torch.nn.functional.unfold(inputs,self.patch_size,stride=self.patch_size).transpose(1,2)
        ...
        inputs = inputs.reshape(B,N,C)
        inputs = torch.nn.functional.fold(inputs.transpose(1,2),inputs_side,self.patch_size,stride=self.patch_size)
    ...
    def __call__(self, x, target, att, un_mixed_prediction_distribution=None):
        ...
        x,target_mask, source_mask = self.tpmix(x, att, rectangle_size)
        ...
        return x, mixed_target, mixed_prediction_distribution
