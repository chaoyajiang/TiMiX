...
class MPLUG(nn.Module):
    ...
    def forward(self, image, text=None, alpha=0, prefix_input=None,  prefix_target=None, is_region_forward=False ,text_ids =None, text_atts=None, text_ids_masked=None, masked_pos=None, masked_ids=None,
                image_atts=None, idx_to_group_img=None, target_bbox=None, is_image=None, ret_bbox_loss=False ,is_mix_up=False, image_embeds=None, label_mixed=None):
        ...
        if is_region_forward:
            ...
            loss_i2t = -torch.sum(F.log_softmax(logits, dim=1) * label_mixed, dim=1).mean()
            loss_t2i = -torch.sum(F.log_softmax(logits.T, dim=1) * label_mixed, dim=1).mean()
            
            loss_ita_mixed = (loss_i2t + loss_t2i) / 2
            return loss_ita_mixed
        text_output = self.text_encoder(text.input_ids, attention_mask=text.attention_mask, return_dict=True)
        text_embeds = text_output.last_hidden_state
        text_feat = F.normalize(self.text_proj(text_embeds[:, 0, :]), dim=-1)
        text_feat_all = allgather(text_feat, torch.distributed.get_rank(), torch.distributed.get_world_size())

        image_embeds,image_embeddings, salient_score = self.visual_encoder.visual(image, skip_last_layer=True, use_checkpoint=self.use_checkpoint, text_embeds = text_embeds[:, 0, :],alpha=0.0)
        ...
        with torch.no_grad():
            ...
            text_feat_all_m = allgather(text_feat_m, torch.distributed.get_rank(), torch.distributed.get_world_size())
            # text_feat_all = torch.cat([text_feat_m.t(), self.text_queue.clone().detach()], dim=1)
            
            image_embeds_m,_,_ = self.visual_encoder_m.visual(image, skip_last_layer=True,
            ...
            image_feat_all_m = allgather(image_feat_m, torch.distributed.get_rank(), torch.distributed.get_world_size())
            # image_feat_all = torch.cat([image_feat_m.t(), self.image_queue.clone().detach()], dim=1)
           

            sim_i2t_m = image_feat_all_m @ text_feat_all_m.T / self.temp
            sim_t2i_m = text_feat_all_m @ image_feat_all_m.T / self.temp
        ...
        return loss_mlm, loss_ita, loss_itm, loss_prefix, salient_score,image_embeddings, F.softmax(sim_i2t, dim=1)
