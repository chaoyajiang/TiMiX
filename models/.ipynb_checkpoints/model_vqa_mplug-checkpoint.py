...
class MPLUG(nn.Module):
    ...
    def get_hotmap(image, question, answer=None):
        ...
        hotmap = self.visual_encoder.visual.get_hotmap(image, skip_last_layer=True, use_checkpoint=self.use_checkpoint, text_embeds = text_embeds[:, 0, :])
        return hotmap       
    ...
    def forward(self, image, question, answer=None, alpha=0, k=None, weights=None, train=True):
        ...
        if train:               
            ...
            text_embeds = text_output.last_hidden_state
            image = image.to(dtype=next(self.parameters()).dtype) 
            image_embeds = self.visual_encoder.visual(image, skip_last_layer=True, use_checkpoint=self.use_checkpoint, text_embeds = text_embeds[:, 0, :], alpha=1.0)
            if self.large:
            ...
            if self.distill:                    
                with torch.no_grad():
                    ...
                    image_embeds_m = self.visual_encoder_m.visual(image, skip_last_layer=True, use_checkpoint=self.use_checkpoint, text_embeds = text_embeds_m[:, 0, :], alpha=1.0) 
