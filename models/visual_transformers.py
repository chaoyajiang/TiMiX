...
def initialize_clip(config, num_patches=240):
    ...
    if config["clip_name"] == "ViT-B-16":
        clip_model, preprocess = clip.load("ViT-B-16.tar", jit=False, pruning_loc = config['pruning_loc'], token_ratio = config['token_ratio'] )
        num_patches = int(config['image_res']*config['image_res']/(16*16))
        pos_embed = nn.Parameter(torch.zeros(num_patches + 1, 768).float())
    elif config["clip_name"] == "ViT-L-14":
        clip_model, preprocess = clip.load("ViT-L-14.tar", jit=False,pruning_loc = config['pruning_loc'], token_ratio = config['token_ratio'])
